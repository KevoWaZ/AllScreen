...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/search/country?country=${country}&type=${"movie"}&page=${1}`);
  ...
  const loadMore = async () => {
    if (currentPage < totalPages) {
      try {
        setLoadingMore(true);
        const response = await fetch(`/api/search/country?country=${country}&type=${"movie"}&page=${currentPage + 1}`);
        const data = await response.json()
